...
//! `GET /{entity-path}/data/{data-id}/docs` returns
//! a self-contained OpenAPI 3.1 specification describing how to interact with
//! the data resource. The documented HTTP methods are derived from the
//! resource's actual read/write capability.
...
#[tokio::test]
async fn read_write_resource_documents_get_and_put() {
    let server = server().await;

    let (status, doc) = get_json(&server, "/sovd/v1/components/ECU/data/rpm/docs").await;

    assert!(status.is_success(), "got {status}");
    assert_eq!(doc["openapi"], "3.1.0");

    let path = &doc["paths"]["/components/ECU/data/rpm"];
    ...
    assert!(path["put"]["requestBody"].is_object());

    // data should carry the resource's own value schema, both ways.
    ...
    let write_data =
        &path["put"]["requestBody"]["content"]["application/json"]["schema"]["properties"]["data"];
    assert_eq!(write_data["properties"]["value"]["type"], "number");
}

#[tokio::test]
async fn read_only_resource_omits_put() {
    let server = server().await;

    let (status, doc) = get_json(&server, "/sovd/v1/components/ECU/data/temp/docs").await;

    assert!(status.is_success(), "got {status}");
    let path = &doc["paths"]["/components/ECU/data/temp"];
    assert!(path["get"].is_object(), "GET must be documented");
    assert!(
        path["put"].is_null(),
        "PUT must NOT be documented for a read-only resource"
    );
}

#[tokio::test]
async fn docs_for_unknown_data_resource_is_not_found() {
    let server = server().await;

    let (status, _) = get_json(&server, "/sovd/v1/components/ECU/data/missing/docs").await;
...
async fn docs_for_unknown_component_is_not_found() {
    ...
    let (status, _) = get_json(&server, "/sovd/v1/components/Nope/data/rpm/docs").await;
