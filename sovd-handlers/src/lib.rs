...
pub fn find_single_process(
    ...
    info!(
        "Starting find_single_process with search_term: '{}' and base_uri: '{}'",
        search_term, base_uri
    );
    ...
    for process in all_processes {
        if let Ok(cmd) = process.cmdline() {
            ...
            if cmd_string.contains(search_term) && process.stat.pid == pid {
                info!(
                    "Found matching process with search_term: '{}' and pid: '{}'",
                    search_term, pid
                );
                ...
                if process.stat.state != 'I' {
                    ...
                    info!(
                        "Process with pid: {} is in idle state, skipping",
                        process.stat.pid
                    );
                }
            }
    ...
    info!(
        "No matching process found for search_term: '{}' with pid: '{}'",
        search_term, pid
    );
